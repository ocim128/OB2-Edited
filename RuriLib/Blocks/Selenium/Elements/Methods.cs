...
namespace RuriLib.Blocks.Selenium.Elements
{
    ...
    public static class Methods
    {
        ...
        public static bool SeleniumExists(BotData data, FindElementBy findBy, string identifier, int index)
        {
            ...
            try
            {
                GetElement(data, findBy, identifier, index);
                data.Logger.Log($"The element with {findBy} '{identifier}' at index {index} exists", LogColors.JuneBud);
                return true;
            }
            catch (NoSuchElementException)
            {
                data.Logger.Log($"The element with {findBy} '{identifier}' at index {index} does not exist", LogColors.JuneBud);
                return false;
            }
            ...
            {
                throw new Exception($"An unexpected error occurred while checking if element with {findBy} '{identifier}' at index {index} exists. {ex.Message}", ex);
            }
        }
        ...
        private static IWebElement GetElement(BotData data, FindElementBy findBy, string identifier, int index)
        {
            ...
            {
                throw new IndexOutOfRangeException($"Could not find element with {findBy} '{identifier}' at index {index}");
            }
        ...
        {
            var browser = GetBrowser(data);
            var elements = findBy switch
            {
                FindElementBy.Id => browser.FindElements(By.Id(identifier)),
                FindElementBy.Name => browser.FindElements(By.Name(identifier)),
                FindElementBy.ClassName => browser.FindElements(By.ClassName(identifier)),
                FindElementBy.TagName => browser.FindElements(By.TagName(identifier)),
                FindElementBy.LinkText => browser.FindElements(By.LinkText(identifier)),
                FindElementBy.PartialLinkText => browser.FindElements(By.PartialLinkText(identifier)),
                FindElementBy.CssSelector => browser.FindElements(By.CssSelector(identifier)),
            ...
            if (!elements.Any())
            {
                throw new NoSuchElementException($"Could not find any element with {findBy} '{identifier}'");
            }
        ...
        private static string GetElementScript(FindElementBy findBy, string identifier, int index)
            => SharedElementHelpers.GetElementScript(findBy, identifier, index);

        private static string BuildSelector(FindElementBy findBy, string identifier)
            => SharedElementHelpers.BuildSelector(findBy, identifier);
