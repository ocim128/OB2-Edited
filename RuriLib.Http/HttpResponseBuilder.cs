...
internal class HttpResponseBuilder : IDisposable
{
    ...
    private static Dictionary<string, string> ParseCookies(string cookieHeader)
    {
        ...
        // Helper local function to parse and add a single cookie segment.
        // This encapsulates the processing logic for one piece of the cookie string.
        void AddCookieFromSegment(string segment)
        {
            ...
            var cookie = ParseSingleCookie(trimmedSegment);
            if (cookie.HasValue)
            {
        ...
        for (var i = 0; i < cookieHeader.Length; i++)
        {
            ...
            {
                // Found a separator, so we process the segment.
                AddCookieFromSegment(cookieHeader.Substring(start, i - start));
                start = i + 1;
            }
        }

        // Process the final segment after the loop finishes.
        AddCookieFromSegment(cookieHeader.Substring(start));
