...
namespace RuriLib.Blocks.Playwright.Page
{
    ...
    public static class Methods
    {
        ...
        public static async Task PlaywrightNavigateTo(BotData data, string url, int timeoutSeconds = 30)
        {
            LogMethodStart(data, $"Navigating to {url}");
            var page = GetPage(data);
            await page.GotoAsync(url, CreatePageOptions<PageGotoOptions>(timeoutSeconds));
            data.Logger.Log($"Navigated to {url}", LogColors.MediumPurple);
        }
