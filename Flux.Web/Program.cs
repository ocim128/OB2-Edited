...
using Flux.Web;
using Flux.Web.Controllers;
using Flux.Web.Exceptions;
using Flux.Web.Interfaces;
using Flux.Web.Middleware;
...
builder.Services.AddScoped<IWordlistRepository>(service =>
    new HybridWordlistRepository(service.GetService<IServiceScopeFactory>(),
...
builder.Services.AddSingleton(_ => new FluxSettingsService(Globals.UserDataFolder));
...
var fluxSettings = app.Services.GetRequiredService<FluxSettingsService>().Settings;
...
if (fluxSettings.SecuritySettings.HttpsRedirect)
{
