    ...
    public partial class HotkeyService : IDisposable
    {
        ...
        private Task HandleCtrlAltQAsync()
        {
            try
            {
                ...
                {
                    // Write the otp.txt file into every running Flux.Native directory
                    var targetDirs = GetFluxDirectories();
                    var writtenCount = 0;
        ...
        private IEnumerable<string> GetFluxDirectories()
        {
            ...
            try
            {
                foreach (var proc in Process.GetProcessesByName("Flux.Native"))
                {
                    try
                    {
