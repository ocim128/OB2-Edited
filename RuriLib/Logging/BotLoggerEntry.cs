...
public class BotLoggerEntry
{
    ...
    public string GetPreview(int maxLength = 100) => string.IsNullOrEmpty(Message) || Message.Length <= maxLength
    ...
    public override string ToString()
    {
        var prefix = Level switch
        {
            ...
            LogLevel.Info => throw new NotImplementedException(),
            _ => "[INFO]"
        };
    ...
    /// </summary>
    public BotLoggerEntry Clone() => new()
    {
        Message = Message,
        Color = Color,
    ...
    /// </summary>
    [JsonIgnore]
    public long EstimatedMemoryBytes => ((Message?.Length ?? 0) * 2) + // Unicode chars = 2 bytes each
                   ((Color?.Length ?? 0) * 2) +
                   ((Category?.Length ?? 0) * 2) +
