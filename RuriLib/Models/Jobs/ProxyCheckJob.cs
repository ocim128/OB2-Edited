...
public class ProxyCheckJob(RuriLibSettingsService settings, PluginRepository pluginRepo, IJobLogger logger = null) : Job(settings, pluginRepo, logger)
{
    /// <summary>
    /// Options
    /// </summary>
    ...
    private readonly Func<ProxyCheckInput, CancellationToken, Task<Proxy>> _workFunction =
    ...
    public override async Task Start(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            parallelizer = ParallelizerFactory<ProxyCheckInput, Proxy>
                ...
                _workFunction, Bots, Proxies.Count(), 0, BotLimit);
    ...
    #endregion Wrappers for TaskManager methods
    ...
    /// <summary>
    /// We&#39;re not logging results to the IJobLogger because they could arrive at a very high rate// and not be very useful, we&#39;re mostly interested in errors here.
    /// </summary>
    private void PropagateResult(object _, ResultDetails<ProxyCheckInput, Proxy> result) => OnResult?.Invoke(this, result);
    ...
    #endregion Propagation of TaskManager events

    #region Private Methods
    private void StartTimer() => _tickTimer = new Timer(new TimerCallback(_ => OnTimerTick?.Invoke(this, EventArgs.Empty)),
            null, (int)TickInterval.TotalMilliseconds, (int)TickInterval.TotalMilliseconds);

    private void StopTimer() => _tickTimer?.Dispose();
    ...
    private void UpdateProxy(object sender, ResultDetails<ProxyCheckInput, Proxy> details)
    {
        ...
        if (proxy.WorkingStatus == ProxyWorkingStatus.Working)
        {
            Working++;
        }
        else
...
public struct ProxyCheckInput(Proxy proxy, string url, string successKey,
    TimeSpan timeout, IProxyGeolocationProvider geoProvider)
{
    ...
    public IProxyGeolocationProvider GeoProvider { get; set; } = geoProvider;
}
