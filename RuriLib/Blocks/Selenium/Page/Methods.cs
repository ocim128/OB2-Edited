...
namespace RuriLib.Blocks.Selenium.Page
{
    ...
    public static class Methods
    {
        ...
        public static void SeleniumNavigateTo(BotData data, string url = "https://example.com", int timeout = 30000)
        {
            ...
            {
                throw new Exception($"The browser is not open. {ex.Message}", ex);
            }
            ...
            {
                throw new Exception($"Failed to navigate to URL: {url}. {ex.Message}", ex);
            }
            catch (Exception ex)
            {
                throw new Exception($"An unexpected error occurred while navigating to URL: {url}. {ex.Message}", ex);
            }
