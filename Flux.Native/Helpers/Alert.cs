...
namespace Flux.Native.Helpers
{
    public static class Alert
    {
        ...
        {
            var fluxSettingsService = App.ServiceProvider.GetRequiredService<Flux.Core.Services.FluxSettingsService>();

            // If the user checked 'don't ask again' for this specific setting
            if (fluxSettingsService.Settings.GeneralSettings.GetProperty(settingName) is bool b && !b)
            {
                return true;
            }
